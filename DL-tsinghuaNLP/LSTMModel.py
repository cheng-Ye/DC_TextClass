...
class LSTMModel(object):
    def __init__(self, n_voc, trainset, testset,classes, prefix):
        if prefix != None:
            prefix += '/'
        ...
        layers = []
        layers.append(EmbLayer(rng, docs, n_voc, 200, 'emblayer', None, prefix))
    ...
    def train(self, iters=None):
        print 'self.trainset.epoch:',self.trainset.epoch
        if not iters:
            lst = range( self.trainset.epoch)
            random.shuffle(lst)
        else:
    ...
    def test(self):
        ...
        for i in xrange(self.testset.epoch):
            ...
            preds.extend(list(tmp[0]))
            label.extend(list(tmp[1]))
            accuracy,f1=accuracy_score(preds,label),f1_score(preds,label,average='weighted')
        print 'Accuracy:',accuracy,'f1_score:',f1
        return preds, label,f1
